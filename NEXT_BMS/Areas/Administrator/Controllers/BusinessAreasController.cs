...
namespace NEXT_BMS.Areas.Administrator.Controllers
{
    ...
    public class BusinessAreasController : Controller
    {
        ...
        public BusinessAreasController(NEXT_BMSContext context)
        {
            _context = context;
        }
        [HttpPost]
        public IActionResult GetBusinessAreas ()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var returnData = (from manudata in _context.BusinessAreas.Where(x=>x.IsDeleted==false) select manudata);
        ...
        // GET: Administrator/BusinessAreas
        ...
        //{
              //return View(await _context.BusinessAreas.Where(x=>!x.IsDeleted).ToListAsync());
        //}

        public async Task<IActionResult> Details(int? id)
        {
            if (id == null || _context.BusinessAreas == null)
            {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,IsActive,IsDeleted")] BusinessArea businessArea)
        {
            if (ModelState.IsValid)
            {
                ...
                TempData["Success"] = "businessArea saved successfully.";
                return RedirectToAction(nameof(Index));
            }
            TempData["Error"] = "An error occured while saving businessArea. Please review your input.";
            return View(businessArea);
        }
        ...
        {
            if (_context.BusinessAreas == null)
            {
                return Problem("Entity set 'NEXT_BMSContext.BusinessAreas'  is null.");
            }
            var businessArea = await _context.BusinessAreas.FindAsync(id);
            if (businessArea != null)
            {
                 businessArea.IsActive = false;
